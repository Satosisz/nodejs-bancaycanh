...
export class StatisticService {
	...
	async getStatistic(filters) {
		...
		let dataStatus = await this.orderRepo.query(`select tst_status, count(*) as total,
			SUM(tst_total_money) as total_price
			FROM transactions where created_at >= '${start_date}' AND created_at <= '${end_date}'
			GROUP BY tst_status`
		);
		...
		const responseGroupDay = await this.orderRepo.query(`select count(*) as total,
			SUM(tst_total_money) as total_price, DATE_FORMAT(created_at,'%Y-%m-%d') as day
			from transactions WHERE tst_status != 3 AND created_at >= '${start_date}' AND created_at <= '${end_date}'
		...
		const totalPriceOrder = await this.transactionRepo.sum('tst_total_money',
			{
				tst_type: 2
			})
