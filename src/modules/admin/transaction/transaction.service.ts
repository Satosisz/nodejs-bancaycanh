...
export class TransactionService {
	...
	async create(createTransactionDto: CreateTransactionDto, user: any) {
		...
		await this.countProduct(products);
	...
	async findOne(id: number) {
		var transaction = await this.transRepo.findOne({
			where: { id: id },
			relations: {
				user: true,
				orders: {
		...
		transaction.orders.forEach(element => {
			element.products.pro_avatar = process.env.BACKEND_APP_URL + "/api/upload/" + element.products.pro_avatar;
			return element;
		});
		return transaction;
	}
	...
	async update(id: number, updateTransactionDto: UpdateTransactionDto) {
		await this.transRepo.update(id, updateTransactionDto);
        return this.transRepo.findOneBy({ id: id });
	}
